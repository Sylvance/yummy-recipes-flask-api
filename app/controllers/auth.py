...
from app.models import User, BannedToken
from app.helpers.response_helper import response, response_auth
from sqlalchemy import exc
from app.helpers.auth_helper import token_required
import re
...
class RegisterUser(MethodView):
    ...
    def post(self):
        """ Register a user, generate their token and add them to the database :return: Json Response with the user`s token """
        if request.content_type == 'application/json':
            ...
            return response('failed', 'Missing or wrong email format or password is less than six characters', 400)
...
class LoginUser(MethodView):
    ...
    def post(self):
        """ Login a user if the supplied credentials are correct. :return: Http Json response """
        if request.content_type == 'application/json':
            post_data = request.get_json()
            email = post_data.get('email')
            password = post_data.get('password')
            if re.match(r"[^@]+@[^@]+\.[^@]+", email) and len(password) > 4:
                ...
                if user and BCRYPT.check_password_hash(user.password, password):
                    return response_auth('success', 'Successfully logged In', user.encode_token(user.id), 200)
                return response('failed', 'User does not exist or password is incorrect', 401)
            return response('failed', 'Missing or wrong email format or password is less than six characters', 401)
...
class LogOutUser(MethodView):
    ...
    def post(self):
        """ Try to logout a user using a token :return: """
        auth_header = request.headers.get('Authorization')
        if auth_header:
            try:
                authentication_token = auth_header.split(" ")[1]
            except IndexError:
                return response('failed', 'Provide a valid auth token', 403)
            else:
                decoded_token_response = User.decode_token(
                    authentication_token)
                if not isinstance(decoded_token_response, str):
                    token = BannedToken(authentication_token)
                    token.banned()
...
@authentication.route('/authentication/reset/password', methods=['POST'])
@token_required
    ...
    if request.content_type == "application/json":
        ...
        if BCRYPT.check_password_hash(current_user.password, old_password):
            ...
            if not len(new_password) > 4:
                return response('failed', 'New password should be greater than six characters long', 400)
...
authentication.add_url_rule('/authentication/register',
                  view_func=registration_view, methods=['POST'])
authentication.add_url_rule('/authentication/login',
                  view_func=login_view, methods=['POST'])
authentication.add_url_rule('/authentication/logout',
                  view_func=logout_view, methods=['POST'])
